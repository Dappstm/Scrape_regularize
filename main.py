...
from config import DEFAULT_OUT_DIR, DEFAULT_DB_PATH, DEFAULT_DOWNLOAD_DIR, DEFAULT_USER_DATA, BROWSER
...
def run(query: str, out_dir: Path, db_path: Path, download_dir: Path, user_data: Path, headless: bool=False):
    ...
    with sync_playwright() as p:
        ...
        reg = RegularizeClient(context, download_dir=download_dir)
        reg.open()
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="PGFN/Regularize mini-program (manual hCaptcha solve, no bypass)")
